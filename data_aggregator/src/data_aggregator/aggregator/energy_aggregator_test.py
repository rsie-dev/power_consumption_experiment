...
def test_aggregate_energy_single(aggregator, single_run_data_frame):
    data = """
host,tool,dataset,mode,strength,threading,run,duration,energy,size
No Unit,No Unit,No Unit,No Unit,No Unit,No Unit,No Unit,second,joules,byte
raspi5,bzip2,sensor,compress,default,single,1,0.07,0.21341973333599978,1.0"""
    df_expected = _as_dataframe(data)

    df_actual = aggregator.aggregate_energy(single_run_data_frame)

    for key in ["duration", "energy"]:
